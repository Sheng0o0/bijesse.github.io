...
3) Attach an onclick event handler to the the element 'roll-dice' - when this button is clicked, it should trigger the above rollDice function
  ...
  i- when the buttong is clicked initilize function using Math random
  ii- call the value of the function to both dice images
  iii- add class to the dice depnding on the number generated
    ...
    function rollDice(){
      ...
      var random2 = Math.floor(Math.random()*6+1);
        ...
        var secondDie = "dice-" + random2; 
          $("#first-die").attr("class", "dice " + firstDie); 
          $("#second-die").attr("class", "dice " + secondDie);
      //for debugging does not add anything to the actual program
